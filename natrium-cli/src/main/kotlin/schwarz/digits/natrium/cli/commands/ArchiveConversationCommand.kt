...
class ArchiveConversationCommand : CliktCommand(name = "conversation-archive") {
    ...
    override fun run() = runBlocking {
        ...
        when (val findResult = session.conversationManager.findConversation(ConversationId.fromString(id))) {
            is FindConversationResult.Success -> {
                when (val result = findResult.conversationOperations.archive()) {
                    is ArchiveConversationResult.Success ->
                        echo("Conversation archived")
                    is ArchiveConversationResult.Failure.NotLoggedIn ->
                        echo("Not logged in")
                    is ArchiveConversationResult.Failure.Unknown ->
